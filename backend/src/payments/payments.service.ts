...
export class PaymentsService {
  ...
  private portOneClient: any;
  private PortOneModule: any;

  constructor(private readonly prisma: PrismaService) {
    this.initializePortOne();
  }

  // 동적 import를 사용하여 PortOne 초기화
  private async initializePortOne() {
    this.PortOneModule = await import('@portone/server-sdk');
    this.portOneClient = this.PortOneModule.PortOneClient({
  ...
  async verifyPayment(verifyPaymentDto: VerifyPaymentDto, userId: string) {
    try {
      // PortOne 클라이언트가 초기화될 때까지 대기
      while (!this.portOneClient) {
  ...
  async handleWebhook(body: string, headers: Record<string, string>) {
    try {
      // 동적 import로 Webhook 가져오기
      while (!this.PortOneModule) {
        await new Promise(resolve => setTimeout(resolve, 100));
      }
      
      const webhook = await this.PortOneModule.Webhook.verify(
        ...
        if (existingPayment) {
          await this.prisma.payment.update({
            ...
            data: {
              ...
              ...(payment.status === 'CANCELLED' && {cancelledAt: new Date()}),
              // portOne 상에서 넘어온 모든 데이터들은 디버깅을 위해서 portoneData로 무조건 전부 다 남겨놓아야 함
            },
          });
...
// 결제 관련 로직은 try-catch로 감싸서 안전하게 해주는게 좋음